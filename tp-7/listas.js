...
function calcularPromedio(listaDeNumeros) {
    var suma = 0;
    var prom;
    var longi = listaDeNumeros.length;
...
function ordenarDeMayorAMenor(listaDeNumeros) {
    var reverso = listaDeNumeros.sort((a,b) => b - a)
    return reverso;
}
...
function ordenarPalabrasPorLongitud(listaDePalabras) {
    var longi = [];
    var lon = listaDePalabras.length;
    for (var i = 0; i < lon; i++) {
        longi[i] = listaDePalabras[i].length;
    }
    longi.sort((a,b) => a - b)
    i = 0;
    var longi2;
    do{
        longi2 = listaDePalabras[i].length;
        if(longi[i] == longi2){
            longi[i] = listaDePalabras[i];
        }
        i++
    }while(longi.length == lon);
    return longi;
}
